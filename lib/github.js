...
class GitHub extends Release {
  ...
  setIsHttps(isHttps){
    ...
    console.log("setIsHttps: "+this.isHttps);
  }
  ...
  get client() {
    ...
    var baseUrl = `https://${isGitHub ? 'api.github.com' : host}${isGitHub ? '' : '/api/v3'}`;
    }else{
    var baseUrl = `http://${isGitHub ? 'api.github.com' : host}${isGitHub ? '' : '/api/v3'}`;  
    }
    const options = {
      baseUrl,
  ...
  getReleaseUrlFallback(version) {
    ...
    if(this.isHttps === true){
      ...
      return `http://${host}/${repository}/releases/tag/${tag}`;
    }
  }
}
